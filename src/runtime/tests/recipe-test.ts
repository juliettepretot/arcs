...
describe('recipe', () => {
  ...
  it('verifies required consume connection is provided by a fulfilled slot', async () => {
    const manifest = await Manifest.parse(`
      particle A
        consume slot1
          must provide slot2
  ...
  it('comments unfulfilled slot connections', async () => {
    ...
    assert.isTrue(recipe.normalize());
    assert.isFalse(recipe.isResolved());
    // TODO(sjmiles): recipe.isResolved() is false, but it's no longer
    // reporting 'unfulfilled slot connections'
    console.log(recipe.toString({showUnresolved: true}));
    /*
    assert.skip.isTrue(recipe.toString({showUnresolved: true}).includes(
