...
export class Particle implements Comparable<Particle> {
  ...
  isResolved(options = undefined) {
    ...
    if (slandleConnections.length ===0 && this.spec.slotConnections.size > 0) {
      const connections = this.getSlotConnections();
      const fulfilledSlotConnections = connections.filter(connection => connection.targetSlot !== undefined);
      if (fulfilledSlotConnections.length < connections.length) {
        if (options && options.showUnresolved) {
