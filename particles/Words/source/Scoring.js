...
class Scoring {
  ...
  static isMinimumWordLength(length) {
    return length >= Scoring.MINIMUM_WORD_LENGTH;
  }
  static pointsForLetter(letter) {
    return Scoring.CHAR_SCORE[letter];
  }
  static wordScore(tiles) {
    return (
        Scoring._wordLengthMultiplier(tiles.length) *
        tiles.reduce(
            (accumulator, t) => accumulator + Scoring.CHAR_SCORE[t.letter], 0));
  }
